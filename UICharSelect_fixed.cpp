...
namespace jrc
{
          ...
          charcount_absolute(c), slots_absolute(s)
    {
        ...
        */
        ...
        auto charinfo_sprite = charselect["charInfo"];
        if (charinfo_sprite.data_type() != nl::node::type::none) {
            sprites.emplace_back(charinfo_sprite, charinfopos);
        } else {
            Console::get().print("[UICharSelect] Warning: Missing charInfo sprite, using fallback");
            // Use a basic frame as fallback
            auto fallback_frame = common["frame"];
            if (fallback_frame.data_type() != nl::node::type::none) {
                sprites.emplace_back(fallback_frame, charinfopos);
            }
        }
        
        auto selectworld_sprite = common["selectWorld"];
        if (selectworld_sprite.data_type() != nl::node::type::none) {
            sprites.emplace_back(selectworld_sprite, selworldpos);
        } else {
            Console::get().print("[UICharSelect] Warning: Missing selectWorld sprite");
        }
        
        // Handle potentially missing world selection sprites
        auto world_icon = charselect["selectedWorld"]["icon"]["15"];
        if (world_icon.data_type() != nl::node::type::none) {
            sprites.emplace_back(world_icon, selworldpos);
        }
        
        auto world_name = charselect["selectedWorld"]["name"]["15"];
        if (world_name.data_type() != nl::node::type::none) {
            sprites.emplace_back(world_name, selworldpos);
        }
        
        auto world_channel = charselect["selectedWorld"]["ch"][channel_id];
        if (world_channel.data_type() != nl::node::type::none) {
            sprites.emplace_back(world_channel, selworldpos);
        }

        emptyslot = charselect["buyCharacter"];
        nametag   = charselect["nameTag"];

        // Handle missing buttons with null checks and fallbacks
        auto btselect_node = charselect["BtSelect"];
        if (btselect_node.data_type() != nl::node::type::none) {
            buttons[BT_SELECTCHAR] = std::make_unique<MapleButton>(btselect_node, charinfopos + Point<int16_t>(-76, 72));
        } else {
            Console::get().print("[UICharSelect] Warning: Missing BtSelect button, using fallback");
            // Try to find alternative buttons or create a basic fallback
            auto fallback_button = common["frame"];
            if (fallback_button.data_type() != nl::node::type::none) {
                buttons[BT_SELECTCHAR] = std::make_unique<MapleButton>(fallback_button, charinfopos + Point<int16_t>(-76, 72));
            }
        }
        
        auto btnew_node = charselect["BtNew"];
        if (btnew_node.data_type() != nl::node::type::none) {
            buttons[BT_CREATECHAR] = std::make_unique<MapleButton>(btnew_node, Point<int16_t>(200, 495));
        } else {
            Console::get().print("[UICharSelect] Warning: Missing BtNew button, using fallback");
            auto fallback_button = common["frame"];
            if (fallback_button.data_type() != nl::node::type::none) {
                buttons[BT_CREATECHAR] = std::make_unique<MapleButton>(fallback_button, Point<int16_t>(200, 495));
            }
        }
        
        auto btdelete_node = charselect["BtDelete"];
        if (btdelete_node.data_type() != nl::node::type::none) {
            buttons[BT_DELETECHAR] = std::make_unique<MapleButton>(btdelete_node, Point<int16_t>(320, 495));
        } else {
            Console::get().print("[UICharSelect] Warning: Missing BtDelete button, using fallback");
            auto fallback_button = common["frame"];
            if (fallback_button.data_type() != nl::node::type::none) {
                buttons[BT_DELETECHAR] = std::make_unique<MapleButton>(fallback_button, Point<int16_t>(320, 495));
            }
        }
        
        auto pagel_node = charselect["pageL"];
        if (pagel_node.data_type() != nl::node::type::none) {
            buttons[BT_PAGELEFT] = std::make_unique<MapleButton>(pagel_node, Point<int16_t>(100, 490));
        } else {
            Console::get().print("[UICharSelect] Warning: Missing pageL button, using fallback");
            auto fallback_button = common["frame"];
            if (fallback_button.data_type() != nl::node::type::none) {
                buttons[BT_PAGELEFT] = std::make_unique<MapleButton>(fallback_button, Point<int16_t>(100, 490));
            }
        }
        
        auto pager_node = charselect["pageR"];
        if (pager_node.data_type() != nl::node::type::none) {
            buttons[BT_PAGERIGHT] = std::make_unique<MapleButton>(pager_node, Point<int16_t>(490, 490));
        } else {
            Console::get().print("[UICharSelect] Warning: Missing pageR button, using fallback");
            auto fallback_button = common["frame"];
            if (fallback_button.data_type() != nl::node::type::none) {
                buttons[BT_PAGERIGHT] = std::make_unique<MapleButton>(fallback_button, Point<int16_t>(490, 490));
            }
        }
